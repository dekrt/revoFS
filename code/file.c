                                   ...
                                   int create)
{
    ...
    struct revofs_file_ei_block *index;
    ...
    if (!bh_index)
        return -EIO;
    index = (struct revofs_file_ei_block *) bh_index->b_data;
...
static void revofs_readahead(struct readahead_control *rac)
{
    mpage_readahead(rac, revofs_file_get_block);
}
#else
 ...
 */
static int revofs_writepage(struct page *page, struct writeback_control *wbc)
{
    return block_write_full_page(page, revofs_file_get_block, wbc);
}
...
#endif
{
    ...
    if (pos + len > REVOFS_MAX_FILESIZE)
        return -ENOSPC;
    nr_allocs = max(pos + len, file->f_inode->i_size) / REVOFS_BLOCK_SIZE;
    ...
    /* prepare the write */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 0)
    err = block_write_begin(mapping, pos, len, pagep, revofs_file_get_block);
#else
    err = block_write_begin(mapping, pos, len, flags, pagep,
                            revofs_file_get_block);
#endif
                              ...
                              void *fsdata)
{
    ...
    inode->i_blocks = inode->i_size / REVOFS_BLOCK_SIZE + 2;
    ...
    if (nr_blocks_old > inode->i_blocks) {
        ...
        first_ext = revofs_ext_search(index, inode->i_blocks - 1);
        /* Reserve unused block in last extent */
        if (inode->i_blocks - 1 != index->extents[first_ext].ee_block)
            first_ext++;

        for (i = first_ext; i < REVOFS_MAX_EXTENTS; i++) {
            if (!index->extents[i].ee_start)
                break;
            put_blocks(REVOFS_SB(sb), index->extents[i].ee_start,
                       index->extents[i].ee_len);
            memset(&index->extents[i], 0, sizeof(struct revofs_extent));
        }
...
#else
    .readpage = revofs_readpage,
#endif
    .writepage = revofs_writepage,
    .write_begin = revofs_write_begin,
    .write_end = revofs_write_end,
};
