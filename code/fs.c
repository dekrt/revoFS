...
/* Global variable for the netlink socket */
struct sock *nlsk = NULL;

/* External reference to the initial network namespace */
...
/* Function to send a user message via netlink */
int send_usrmsg(char *pbuf, uint16_t len)
{
...
/* Callback function for receiving netlink messages */
static void netlink_rcv_msg(struct sk_buff *skb)
{
...
static int __init revofs_init(void)
{
    ...
    nlsk = (struct sock *) netlink_kernel_create(&init_net,
                                                 NETLINK_MSG_FOR_SCHIPS, &cfg);
    if (nlsk == NULL) {
...
static void __exit revofs_exit(void)
{
    ...
    /* Release the netlink socket if it exists */
    if (nlsk) {
...
MODULE_DESCRIPTION("a new-design file system");
