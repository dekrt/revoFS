...
以下是项目的目标描述 <font color="#9933FF"><b>(全部已完成)</b></font> 

- **目标1**：将新创建的文件系统的**操作接口和VFS对接**。	**（已完成）**
- **目标2**：实现新的文件系统的**超级块、dentry、inode的读写操作**。	**（已完成）**
- **目标3**：实现新的**文件系统的权限属性**，不同的用户不同的操作属性。**（已完成）**
- **目标4**：实现和**用户态程序的对接**，用户程序	**（已完成）**
- **目标5**：实现将一个**块设备**（可以用文件模拟）**格式化**成自己设计的文件系统的格式。	**（已完成）**
- **目标6**：设计一个**用户态的测试用例应用程序**，验证文件系统`open/read/write/ls/cd `等通常文件系统的访问。 **（已完成）**

| 编号 | 完成情况说明 |
| ---- | -------- |
| 1    | 实现了文件系统与VFS的对接。具体来说，编写了一组满足VFS接口要求的函数，包括但不限于super_operations、inode_operations和file_operations等。这些函数实现了文件系统的基本操作，并已被成功注册到VFS。 |
| 2    | 实现了超级块、dentry、inode的读写操作。操作包括读取和修改元数据，创建和删除目录项和索引节点等。 |
| 3    | 文件系统支持Linux的标准文件权限模型，包括用户、组和其他的读、写、执行权限。 |
| 4    | 能够通过mount命令把格式化好的块设备安装到/mnt，能够用ls cat等命令查看目录文件。 |
| 5    | 实现了将一个块设备格式化成文件系统的格式。编写了一个名为mkfs的工具，它可以将一个块设备或者一个文件初始化成文件系统格式。 |
| 6    | 设计了一个用户态的测试用例应用程序，它可以执行open、read、write、ls、cd等常见的文件系统操作。在我们的文件系统上运行了这个程序，并且所有的测试用例都通过了。 |
...
以下是文件系统测试目标描述 <font color="#9933FF"><b>(全部已完成)</b></font> 
